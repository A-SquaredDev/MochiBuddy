...
Status: DONE 2026-08-01. The tired and sleeping z's stroke a pre-traced unit path scaled by font size, ending per-frame Text glyph layout (the file's hottest cost). All five idle canvases take a paused input; the Home hero derives it from scenePhase, tab selection, and the three sheets, so a covered or off-tab hero stops ticking. Cadence capped at 60fps (30fps for the near-static unwell and sleeping). The outer saturation filter is structurally dropped at full color and for the alive unwell canvas, which folds mood saturation into its own fever-fade filter (one color pass, never two; transitions into full color now snap saturation, dwarfed by the simultaneous face swap). Constant geometry hoisted to file scope (grin, tongue, lids, lashes, lips, drips, brows, grimace, stroke styles); parametrized helpers translate a shared path instead of rebuilding. Reduce Motion falls back to the static pose and stops every repeatForever (bobbing, sparkles, sleep-z drift, skeleton shimmer). The widget's 3x ImageRenderer moved behind a per (mood, theme, size) cache. Verified: full suite 780 passed, 0 failed. Two follow-ups: eyeball the stroked z against the old glyph in the All states preview, and an Instruments before/after on Home is the real acceptance test for the frame cost.
...
Status: DONE 2026-08-01, all three phases plus a network meter. Phase 1: ObservationService memoizes the 132-day scan and list ids per (userId, civil day), invalidated by every task mutation via CachingTaskRepository.onMutation (profile half deliberately not memoized so intervals stay live); CachingUserProfileRepository background refresh honors a 5-minute TTL on an injectable clock; completions issue ONE merged coins+streak write (new applyCompletion on the protocol); the letter archive existence check memoizes per period; ensureMoment writes blindly (rejected create-only writes are free). Phase 2: CachingTaskRepository, a write-through decorator over the full task protocol with provable coverage math for completed-window and limit queries, stat synthesis on completion, a version clobber guard, and structural per-uid scoping. Phase 3: RootView is the sole foreground worker (drain, entitlement, one task-cache refresh) and a foregroundPulse on TabCoordinator tells Home and Tasks to re-derive from warm caches in order; their scenePhase handlers are gone. NetworkCallMeter counts every Firestore read and write call at the FirestoreReadLog choke points (recordWrite added at every write site), day-keyed with a structural midnight reset, displayed with Refresh/Zero in the DevScheduler screen. Verified: full suite 809 passed, 0 failed, including ~28 new tests (task cache, inputs memo, profile TTL, meter, one-write completion, letter memo). Aaron measures the real-world reduction by reading the meter across a normal day before and after this branch.

Deferred follow-ups discovered here: lists queries are still uncached (small docs, several fetches per foreground); the unread-envelope refresh reads the whole letter archive once per foreground by design (cacheable if the archive grows); cancelled editor dismissals still trigger a (now cheap, cache-served) refresh plus relay.
