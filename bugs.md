...
**Notes:** Confirmed to be the Apple limitation: the name comes over on the very first authorization only, and `saveAccountLink` already persists it when it arrives. Delete the account and sign in again and Apple sends nothing, so the fresh profile document has no display name to show. Discussed 2026-08-05, decided: ask, do not invent. Entering Home with an empty display name now presents a one-time sheet, "What should {PetName} call you?", with a "Not now" that is a real exit ("Mochi friend" stays the fallback). Saving closes it structurally; skipping records a per-UID device-local flag (DisplayNamePromptGate, sibling of NudgeLedger) that account deletion clears. Chosen over a full onboarding-style screen because the app behind it is already usable - this is a gap being filled, not a step being taken.
