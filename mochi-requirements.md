...
> 🛠 **As of July 24 2026 (v0.7 · Features 1 + 4 + 3 + 2).** Everything the changelog
> resolved through v0.6.1, plus Personal Layer Features 1, 4, 3, and 2 (the first four
> stops in the pinned build order 1 → 4 → 3 → 2 → 5 → 6), is built and covered by the
> automated suite: **605 tests passed, 0 failures** (`MochiBuddyTests`, Swift Testing,
...
> Feature 2 needs no rules change. Remaining user-owned console/account work is tracked
> in *Outstanding external setup* below. Next up: Feature 5 (suggested times).
...
- [ ] **Remote Config: create the 36 pending parameters** (Firebase console →
  Remote Config → publish). The 24 `obs_*`, 6 `letter_*`, and 6 `callback_*`
  (Feature 2, added July 24 2026) Number keys listed above, each set to its
  shipped default so behavior is unchanged until a deliberate tuning pass.
  `RemoteTuning.numberKeys` is the canonical list and the `consoleKeysMatch`
  test pins all 60 keys - run the suite after publishing to confirm nothing
  drifted.
